							      ...
							      uint16_t mechanical_angle_u16)
{
	...
	/* Convert angle-count delta per sample into signed mechanical speed in mrpm. */
	int64_t mechanical_speed_mrpm_num = (int64_t)mechanical_angle_delta *
										MOTOR_SPEED_ESTIMATOR_MS_PER_MINUTE *
										MOTOR_SPEED_ESTIMATOR_MRPM_PER_RPM;
	int64_t mechanical_speed_mrpm_den = MOTOR_SPEED_ESTIMATOR_MECHANICAL_TURN_COUNTS_U16 *
										(int64_t)motor_speed_estimator_h->cfg->sample_period_ms;
	int32_t estimated_mechanical_speed_mrpm = (int32_t)(mechanical_speed_mrpm_num /
														mechanical_speed_mrpm_den);
